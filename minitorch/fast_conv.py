                ...
                for ic_ind in range(in_channels):
                    for kw_ind in range(kw):
                        ...
                        if 0 <= in_pos < width:
                            ...
                            input_index = (bind*s1[0]) + (ic_ind*s1[1]) + (in_pos*s1[2])
                            # weight index is jumpy by out_channel number, jump by in_channel number, jump by position in kernel
                            weight_index = (oc_ind*s2[0]) + (ic_ind*s2[1]) + (kw_ind*s2[2])
                            oval += input[input_index] * weight[weight_index]

                out_index = ((bind*out_strides[0]) + (oc_ind*out_strides[1]) + (ow_ind*out_strides[2]))
                out[out_index] = oval
                        ...
                        for kri in range(kh):
                            for kci in range(kw): 
                                ...
                                if 0 <= iri < height and 0 <= icoi < width:
                                    ...
                                    input_idx = (bi*isb)+(ici*isc)+(iri*isr)+(icoi*isc)
                                    weight_idx = (oci*wsoc)+(ici*wsic)+(kri*wsr)+(kci*wsc)
                                    # dot product
